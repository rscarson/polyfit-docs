...
function glossarySearch(filterElement: HTMLInputElement) {
    ...
    document.querySelectorAll<HTMLElement>('.accordion-item').forEach(item => {
        ...
        const cards = item.querySelectorAll<HTMLElement>('.card');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    search?.addEventListener('keyup', (e) => glossarySearch(e.target as HTMLInputElement));

    // On enter, scroll to first highlighted item
    search?.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
            const firstHighlight = document.querySelector<HTMLElement>('.card[style*="border: 2px solid orange"]');
            if (firstHighlight) {
...
function scrollToAnchor(anchor: string, no_scroll = false) {
    if (!anchor) return;
    ...
    if (anchorElement) {
        ...
        while (parent) {
            ...
            const p = parent.parentElement as HTMLElement;
            parent = p;
        }
    }
...
function scrollToHighlight(e: Event) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll<HTMLAnchorElement>('.pagelink').forEach(anchor => {
